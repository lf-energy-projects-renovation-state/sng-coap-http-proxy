...
class CoapConfiguration(
    ...
    fun serverConfiguration(): CaliforniumConfiguration =
        CaliforniumConfiguration.getStandard().apply {
    ...
    @Bean
    fun coapEndpoint(
        config: CaliforniumConfiguration,
        dtlsConnector: DTLSConnector,
    ): CoapEndpoint = CoapEndpoint.Builder().setConfiguration(config).setConnector(dtlsConnector).build()

    @Bean
    fun dtlsConnector(
        config: CaliforniumConfiguration,
        remotePskStore: AdvancedPskStore,
    ) = DTLSConnector(
